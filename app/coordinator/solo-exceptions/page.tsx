...
export default function SoloProjectExceptions() {
  ...
  const confirmToggleException = async () => {
    ...
    try {
      ...
      toast.success(
        selectedStudent.newAllowStatus ? "تم منح الاستثناء للطالب بنجاح" : "تم إلغاء الاستثناء من الطالب بنجاح",
      )
      ...
      toast.error("حدث خطأ أثناء تحديث الاستثناء")
    } finally {
      ...
      <div className="p-8 space-y-8">
        <div>
          <h1 className="text-4xl font-bold bg-gradient-to-l from-primary to-primary/60 bg-clip-text text-transparent">
            استثناءات المشاريع الفردية
          </h1>
          <p className="text-muted-foreground mt-2">إدارة الطلاب المصرح لهم بتقديم مشاريع فردية</p>
        </div>
