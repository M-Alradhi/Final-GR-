...
export default function BrowseProjectIdeas() {
  ...
  const handleViewIdea = async (idea: Idea) => {
    try {
      ...
      if (!snap.exists()) {
        toast.error("هذه الفكرة لم تعد موجودة")
        return
      }
      ...
      console.error(e)
      toast.error("حدث خطأ أثناء تحميل تفاصيل الفكرة")
    }
  }
  ...
  const checkIfIdeaSelected = (idea: Idea | null) => {
    ...
    if (mySelection) return { selected: true, byMe: true, message: "تم اختيار هذه الفكرة من قبلك" }
    if (hasOtherSelection) return { selected: true, byMe: false, message: "تم أخذ هذه الفكرة من قبل طالب آخر" }
  ...
  const handleSelectIdea = async () => {
    ...
    if (!canPickAnotherIdea) {
      toast.error("لا يمكنك اختيار أكثر من فكرة. لديك فكرة مختارة بالفعل.")
      return
    }
    ...
    try {
      ...
      if (msg.includes("NOT_AVAILABLE")) {
        toast.error("تم أخذ هذه الفكرة من قبل طالب آخر")
      } else if (msg.includes("ALREADY_HAVE_IDEA")) {
        toast.error("لا يمكنك اختيار أكثر من فكرة. لديك فكرة مختارة بالفعل.")
      } else if (msg.includes("IDEA_NOT_FOUND")) {
        toast.error("الفكرة غير موجودة")
      } else if (msg.includes("USER_NOT_FOUND")) {
        toast.error("حسابك غير موجود في قاعدة البيانات")
      } else {
      ...
      <Card className="rounded-xl hover:shadow-lg transition-all duration-300">
        <CardHeader>
          <div className="flex items-start justify-between">
            ...
            >
              {ideaStatus.selected ? (ideaStatus.byMe ? "مختارة" : "مأخوذة") : "متاح"}
            </Badge>
          </div>
        </CardHeader>
  ...
  const myIdeaBadgeText = useMemo(() => {
    ...
    if (!mySelectedIdea) return "لا يوجد"
    return "مختارة"
      ...
      <div className="p-8 space-y-8 animate-in fade-in duration-500">
        <div className="animate-in slide-in-from-top duration-700">
          ...
          <p className="text-muted-foreground mt-2">
            تصفح واختر من أفكار المشاريع المطروحة من المشرفين (فكرة واحدة فقط)
          </p>
        </div>
        ...
        <Card className="rounded-xl border-2">
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              <CardTitle className="text-lg">فكرتي المختارة</CardTitle>
              <CardDescription>تظهر هنا الفكرة التي اخترتها (إن وجدت)</CardDescription>
            </div>
          ...
          <CardContent>
            {myIdeaLoading ? (
              ...
              <p className="text-sm text-muted-foreground">لم تقم باختيار أي فكرة بعد.</p>
            ) : (
              <div className="space-y-3">
                ...
                <div className="grid md:grid-cols-2 gap-3 text-sm">
                  <div className="flex items-center justify-between">
                    <span className="text-muted-foreground">المشرف:</span>
                    <span className="font-medium">{mySelectedIdea.supervisorName || "غير محدد"}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-muted-foreground">القسم:</span>
                ...
                <CardContent>
                  ...
                  {!canPickAnotherIdea && (
                    <p className="text-xs text-destructive mt-2">
                      لديك فكرة مختارة بالفعل، لا يمكنك اختيار فكرة أخرى.
                    </p>
                  )}
                </CardContent>
              </Card>
            ...
            {availableIdeas.length === 0 ? (
              <Card className="border-dashed">
                <CardContent className="flex flex-col items-center justify-center py-16">
                  ...
                  <p className="text-sm text-muted-foreground">لم يتم طرح أي أفكار متاحة من المشرفين بعد</p>
                </CardContent>
              </Card>
            ) : (
